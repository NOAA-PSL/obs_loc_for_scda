...
def main():
    ...
    ds = xr.open_dataset(my_data_dir+'/temperature_covariances_averaged.nc', chunks={'lat':2, 'lon':72})
    #ds.chunk({lat:5, lon:10, atm_lev=-1, ocn_lev=-1, atm_lev_copy:-1, ocn_lev_copy:-1})

    
    # Store lat/lon pairs
    lats = ds['lat'].values[arg:arg+2]
    lons = ds['lon'].values[:3]
    ...
    #with Client() as c:
    #    ds = c.scatter(ds, broadcast=True)
    ...
    #futures = client.map(compute_loc, lat_lon_list, ds=ds)
    ...
    #ds_future = client.scatter(ds, broadcast=True)
    
    #for ind in range(len(lat_lon_list)):
        # Submit tasks to the dask client in parallel
        #future = client.submit(compute_loc, ds_future, lat_lon_list[ind])
